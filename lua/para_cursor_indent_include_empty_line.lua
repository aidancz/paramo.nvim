local H = vim.deepcopy(require("virtcol"))

H.virtcol_first_nonblank = function(lnum)
	local line = vim.fn.getline(lnum)
	local col1, col2 = string.find(line, "%S")
	if col1 == nil then return nil end
	return vim.fn.virtcol({lnum, col1}, true)[1]
end

H.indent = function(lnum)
	local virtcol_first_nonblank = H.virtcol_first_nonblank(lnum)

	local indent
	if virtcol_first_nonblank then
	-- has non-blank char
		indent = virtcol_first_nonblank - 1
	elseif vim.fn.getline(lnum) == "" then
	-- empty
		indent = -1
	else
	-- only whitespace
		indent = vim.fn.virtcol({lnum, "$"}) - 1
	end

	return indent
end

----------------------------------------------------------------
local f = H.indent
H.indent = function(lnum)
	local indent = f(lnum)
	if indent == -1 then
		local pos_cursor = H.get_cursor()
		local indent_cursor = f(pos_cursor.lnum)
		if indent_cursor == -1 then
			indent = pos_cursor.virtcol - 1
		else
			indent = indent_cursor
		end
	end
	return indent
end
----------------------------------------------------------------

local M = {}

M.is_head = function(pos, eq)
	eq = eq or function(a, b) return a == b end
	local indent_cursor = H.indent(vim.fn.line("."))

	if
		(
			eq(H.indent(pos.lnum), indent_cursor)
		)
		and
		(
			next(H.prev_pos(pos)) == nil
			or
			H.indent(H.prev_pos(pos).lnum) ~= H.indent(pos.lnum)
		)
	then
		return true
	else
		return false
	end
end

M.is_tail = function(pos, eq)
	eq = eq or function(a, b) return a == b end
	local indent_cursor = H.indent(vim.fn.line("."))

	if
		(
			eq(H.indent(pos.lnum), indent_cursor)
		)
		and
		(
			next(H.next_pos(pos)) == nil
			or
			H.indent(H.next_pos(pos).lnum) ~= H.indent(pos.lnum)
		)
	then
		return true
	else
		return false
	end
end

M.is_head_or_tail = function(pos, eq)
	eq = eq or function(a, b) return a == b end
	return M.is_head(pos, eq) or M.is_tail(pos, eq)
end

return M
